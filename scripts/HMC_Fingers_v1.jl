...
function main(;nt=1, tol=1e-3, max_its=1e5)
    # Physical Parameters
    ϕ_amb                  = 0.01
    ...
    for it=1:nt
        ...
        Pf_old  .= Pf
        max_η   = maximum(η)
        @show dτ      = 0.1*Δ.y^2/maximum(η) * 6e4                               # pseudo timestep for solid velocity
        @show dτ_P    = 25. *a.*maximum(η).*Δ.y/Lx   / 1e3
        @show dτ_Pf   = 0.01*Δ.y^2/maximum((k_darcy.*ϕ.^3 .* Ks_mod)/1e-3) * 5e4         # pseudo timestep for fluid pressure
        @show dτ_ϕ    = dt /10  
        errVx0, errVy0 = 1., 1.
        for iter=1:max_its
        ...
        p4 = heatmap!(xv, yv, ρ_f'.*ρ_db, c=:turbo, title="ρ_f")
        p4 = plot!(X_incl, Y_incl, c=:white, label=:none)
        display(plot(p1, p2, p3, p4))
    end
end

main(nt=1, tol=1e-3, max_its=1e5)